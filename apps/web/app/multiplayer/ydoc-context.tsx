...
import ChatBox from "../schema/[id]/components/chatbox";
import {getCustomSchemaAsUpdate, getSchemaAsUpdate} from "../schema/[id]/doc-utils";
...
export const YDocProvider = ({
    ...
    const handleOutputUpdate = async (output: string) => {
        isSavingState[1](true);
        madeChangesState[1](false);
        const {_final_schema, encoded} = getCustomSchemaAsUpdate(output)
        const _schema = ydoc.getText('schema');
        _schema.delete(0, _schema.length)
        _schema.insert(0, _final_schema.toJSON())
        await mutate({
            docState: encoded,
            id: room,
        });
    ...
    return (
        <>
            ...
            >
                {children}
                <ChatBox onOutputUpdate={handleOutputUpdate}/>
            </multiplayerContext.Provider>
        </>
    );
};
